...
public class AliyunOSSClientUtil {
     ...
     */
    public static DataResult<String> uploadPub(MultipartFile multipartFile, Boolean fullPath) {
        try {
            return DataResult.ok(ossUpload(multipartFile, getOSSClient(), fullPath));
        } catch (Exception e) {
            log.error(e.toString());
            return DataResult.fail(CommonConstant.SYS_ERROR, e.getMessage());
        }
    }

    private static String ossUpload(MultipartFile multipartFile, OSSClient ossClient, Boolean fullPath) {
    ...
    public static DataResult<List<String>> uploadPubs(MultipartFile[] multipartFiles, Boolean fullPath) {
        try {
            List<String> result = new ArrayList<>(multipartFiles.length);
            List<MultipartFile> list = Arrays.asList(multipartFiles);
            list.parallelStream().forEach(file -> result.add(ossUpload(file, getOSSClient(), fullPath)));
